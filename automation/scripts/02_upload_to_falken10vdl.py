...
def setup_git_authentication():
    ...
    try:
        ...
        if result.returncode != 0:
            print(
                "ℹ️ Not in a git repository, initializing temporary git repo for tag operations..."
            )
            subprocess.run(["git", "init"], capture_output=True, cwd=os.getcwd())
            subprocess.run(
                ["git", "remote", "add", "origin", bonsaiPR_repo_url],
                capture_output=True,
                cwd=os.getcwd(),
            )
        else:
            ...
            subprocess.run(
                ["git", "remote", "set-url", "origin", bonsaiPR_repo_url],
                capture_output=True,
                cwd=os.getcwd(),
            )
...
def cleanup_local_tag(tag_name):
    ...
    try:
        ...
        if result.stdout.strip():
            ...
            subprocess.run(
                ["git", "tag", "-d", tag_name], capture_output=True, cwd=os.getcwd()
            )
...
def update_release_body(release_id, release_name, release_body):
    ...
    if response.status_code == 200:
        ...
        print(
            f"❌ Failed to update release body: {response.status_code} {response.text}"
        )
        return None
    ...
    try:
        with open(file_path, "rb") as f:
            ...
            response = requests.post(
                url, headers=headers, params=params, data=f, timeout=300
            )
...
def append_upload_info_to_readme(report_file, release_url, tag_name, addon_files):
    ...
    with open(report_file, "a", encoding="utf-8") as f:
        ...
        f.write(
            f"**Upload Date**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}\n"
        )
        ...
        f.write(
            f"- 🌐 [All Releases](https://github.com/{GITHUB_OWNER}/{GITHUB_REPO}/releases)\n\n"
        )
...
def create_or_update_readme():
    ...
    if not readme_path:
        ...
        readme_filename = (
            f"README-bonsaiPR_{pyversion}-{version}-alpha{current_date}.txt"
        )
    ...
    with open(readme_path, "a" if file_exists else "w") as f:
        ...
        addon_files = (
            glob.glob(os.path.join(built_addons_path, "*.zip"))
            if os.path.exists(built_addons_path)
            else []
        )
        ...
        f.write(f"**Release Tag**: {tag_name}\n")
        f.write(
            f"**GitHub Release**: https://github.com/{GITHUB_OWNER}/{GITHUB_REPO}/releases/tag/{tag_name}\n"
        )
        f.write(f"**Release Type**: Pre-release (Alpha)\n\n")
        ...
        if addon_files:
            ...
            for addon_file in sorted(addon_files):
                ...
                f.write(
                    f"- {platform_icon} **{platform_name}**: `{filename}` ({filesize:,} bytes)\n"
                )
        else:
        ...
        if report_file:
            ...
            f.write(f"\n**Documentation**:\n")
            f.write(
                f"- 📄 **Detailed Report**: `{report_filename}` ({report_filesize:,} bytes)\n"
            )

        f.write(f"\n## 🌐 Download Links\n\n")
        f.write(f"**Direct Downloads**:\n")
        f.write(
            f"- 🔗 [GitHub Releases Page](https://github.com/{GITHUB_OWNER}/{GITHUB_REPO}/releases)\n"
        )
        f.write(
            f"- 🔗 [Latest Release](https://github.com/{GITHUB_OWNER}/{GITHUB_REPO}/releases/tag/{tag_name})\n"
        )
        f.write(
            f"- 🔗 [Source Code Branch](https://github.com/{FORK_OWNER}/{FORK_REPO}/tree/{get_branch_name()})\n\n"
        )
        ...
        if report_file and os.path.exists(report_file):
            try:
                ...
                if total_prs > 0:
                    success_rate = (
                        (successfully_merged / total_prs * 100) if total_prs > 0 else 0
                    )

                    # Calculate detailed breakdown to match the old format
                    base_conflicts = report_content.count("Conflict with base branch")
                    pr_conflicts = report_content.count(
                        "Conflict with other merged PRs"
                    )
                    merge_failures = (
                        report_content.count("Automatic merge failed")
                        + report_content.count("Unrelated histories")
                        + report_content.count("Git operation failed")
                    )
                    ...
                    deleted_count = report_content.count(
                        "Repository no longer accessible"
                    )
                    missing_info_count = report_content.count(
                    ...
                    if merge_failures > 0:
                        f.write(
                            f"- **Failed to Merge (other issues)**: {merge_failures}\n"
                        )
                    if other_failures > 0:
                        f.write(f"- **Failed to Merge (unknown)**: {other_failures}\n")

                    f.write(
                        f"- **Skipped (draft/repo issues)**: {draft_count + deleted_count + missing_info_count + other_skipped}\n"
                    )
        ...
        f.write(
            f"- Report issues at: https://github.com/{GITHUB_OWNER}/{GITHUB_REPO}/issues\n"
        )
        f.write(
            f"- Contribute to IfcOpenShell: https://github.com/{SOURCE_REPO_OWNER}/{SOURCE_REPO_NAME}\n\n"
        )

        f.write(f"## ⚠️ Important Notices\n\n")
        f.write(
            f"- **Alpha Release**: This is a development build with experimental features\n"
        )
        ...
        f.write(
            f"- **Backup First**: Always backup your Blender projects before testing\n"
        )
        f.write(
            f"- **Report Issues**: Help improve BonsaiPR by reporting bugs and issues\n\n"
        )

        f.write(f"## 📅 Next Release\n\n")
        f.write(
            f"- **Schedule**: Next automated build will be available next Sunday at 2 AM UTC\n"
        )
        f.write(f"- **Updates**: New PRs merged since this build will be included\n")
        f.write(
            f"- **Notifications**: Watch the repository for release notifications\n"
        )
...
def generate_release_body(
    report_file_path, addon_files, timestamp_from_readme=None, tag_name=None
):
    ...
    try:
        ...
        for idx, line in enumerate(lines):
            ...
            # Example row:
            # | [#7802](url) | title | author | branch | [sha](url) | reason | ...
            if line.startswith("| [#"):
                # Skip header separator rows if any
                ...
                # Commit column differs by section
                commit_cell = None
                ...
                last_commit = None
                if commit_cell:
                    commit_match = re.match(r"\[([0-9a-fA-F]+)\]\(([^)]+)\)", commit_cell)
                    if commit_match:
                        last_commit = {
                ...
                if in_skipped_section:
                    ...
                    if reason and "DRAFT status" in reason:
                        skipped_draft_prs.append(entry)
                    elif reason == "Merges cleanly against base (conflict with other PRs)":
                        skipped_conflict_prs.append(entry)
                    else:
                        failed_prs.append(
                            {
                                "line": parsed_line,
                ...
                if in_applied_section:
                    ...
                    for bl in block_lines:
                        if bl.startswith("- Reason:") or bl.startswith("  - Reason:"):
                            ...
                            "- First detected failing:"
                        ) or bl.startswith("  - First detected failing:"):
        ...
        for addon_file in addon_files:
            ...
            if "windows" in renamed_filename.lower():
                ...
                "macosm1" in renamed_filename.lower()
                or "arm64" in renamed_filename.lower()
            ):
            ...
            downloads_section += (
                f"- **{platform}**: [{renamed_filename}]({download_url})\n"
            )
        ...
        merge_order_banner = (
            ...
            f"If a PR you need is listed under *\u201cSkipped \u2013 Conflict with other PRs\u201d* below, "
...
def cleanup_old_releases():
    ...
    try:
        ...
        if len(versioned_releases) <= 30:
            print(
                f"✅ Found {len(versioned_releases)} releases (≤30), no cleanup needed"
            )
            return
        ...
        print(
            f"✅ Cleanup complete: {deleted_count}/{len(releases_to_delete)} releases deleted"
        )
...
def upload_to_falken10vdl():
    ...
    if not os.path.exists(built_addons_path):
        print(
            f"Error: The built addons directory '{built_addons_path}' does not exist."
        )
        return False
    ...
    if report_file:
        print(
            f"Found report file: {os.path.basename(report_file)} (will be used for release description)"
        )
    else:
    ...
    if report_file and os.path.exists(report_file):
        with open(report_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                elif line.startswith("Merge Order:"):
                    merge_order = line.split(":", 1)[1].strip().split("(")[0].strip()
                # Stop reading once we reach the first section header
                if line.startswith("## "):
                    break

    # Format commit hash as a short hash and link, using the short hash from the GitHub commit page if possible
    commit_url = (
        f"https://github.com/{SOURCE_REPO_OWNER}/{SOURCE_REPO_NAME}/commit/{commit_hash}"
    ...
    ts_full = (
        timestamp_from_readme
        if timestamp_from_readme
    ...
    if branch_name != "unknown":
        ...
        try:
            ...
            if resp.ok:
                ...
                branch_short_hash = (
                    branch_commit_hash[:7]
                    if branch_commit_hash not in (None, "unknown")
                    else "unknown"
                )
        except Exception as e:
    ...
    release_body_header += f"Merge order: {order_label_header}\n"
    ...
    release_body = (
        ...
        + generate_release_body(
            report_file, addon_files, timestamp_from_readme, tag_name
        )
    )
...
def patch_existing_release(tag_name):
    ...
    if not report_file:
        ...
        if all_reports:
            ...
            print(
                f"⚠️  No report matched tag timestamp, using most recent: {os.path.basename(report_file)}"
            )
    ...
    if assets_resp.status_code == 200:
        addon_files = [
            a["name"] for a in assets_resp.json() if a["name"].endswith(".zip")
        ]

    new_body = generate_release_body(
        report_file, addon_files, timestamp_from_readme=timestamp, tag_name=tag_name
    )
