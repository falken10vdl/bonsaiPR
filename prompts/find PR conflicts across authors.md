...
> **PR conflict analysis — IfcOpenShell/IfcOpenShell**
>
...
> 1. Pull **all** open PRs via `gh pr list --state open --json number,title,author,createdAt,isDraft --limit 2000`. Split into {AUTHOR}'s non-draft PRs vs everyone else's non-draft PRs. Report the author breakdown (`Counter` of logins) so the scope is visible.
...
> 3. **Verify every candidate with a real 3-way merge**: fetch both PR heads (`git fetch origin +refs/pull/N/head:refs/prs/N`, only the ones not already fetched) and run `git merge-tree --write-tree --name-only refs/prs/OTHER refs/prs/AUTHOR`. Exit code 1 = real conflict; keep only these. (This dropped most false positives for us — 51 candidates → 7 real.)
...
>
> If (and only if) I approve: verify `gh auth status` is the intended account first. **Then, before posting, check that PR for an existing heads-up** — pull its comments (`gh pr view N --repo {REPO} --json comments`) and skip any PR that already has a conflict heads-up from the authenticated account (match on the "Just a heads up" / `git merge-tree` wording). No need to ping a PR twice; if a prior note is now stale (different files/counts), edit it (`gh pr comment N --edit-last`) rather than adding a new one. Post this template on the **other author's** PR, linking {AUTHOR}'s PR (reference multiple {AUTHOR} PRs in one comment if the same PR conflicts with several):
>
