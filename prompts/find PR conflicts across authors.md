...
>
> If (and only if) I approve: verify `gh auth status` is the intended account first, then post this template on the **other author's** PR, linking {AUTHOR}'s PR (reference multiple {AUTHOR} PRs in one comment if the same PR conflicts with several):
>
