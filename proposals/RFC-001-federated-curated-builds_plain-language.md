...
**Second, the build also carried 81 commits that weren't merges of a proposed change.** The first version of the tool reported all 81 as original work the user had never shared. That was wrong, and the user caught it: 75 of them had already been offered to the project and then copied onto the personal build, which changes their identifier and made them look new. Only **6** had genuinely never left the machine, and they were small follow-up fixes rather than features.

The tool now separates the two, and the honest version of the payoff is narrower but still real: it reconstructs **where every piece of a personal build came from** — what is already upstream, what was taken from someone's proposal, and what exists nowhere else. Nobody records that by hand, and without it a build is just a pile of commits. The lesson is worth keeping too: a measurement that flatters the idea it was built to support deserves a second look before it goes in a document.
...
**When a change breaks, fall back rather than dropping it.** If a contributor's latest version no longer fits, the build quietly uses the last version the curator had verified — and says so in its report, along with how far behind that version now is. The build keeps working, *and* the report doubles as a list of contributors worth telling that their work has stopped fitting. That distance matters, and getting it right took two corrections: it counts the change's *own* work rather than everything that happened upstream since, and it compares content rather than identifiers, because a contributor who rewrites their history makes every commit look new. On the current build four are at zero — rewritten but unchanged in substance — most of the rest are two or three, and one has moved 27, which is a genuine gap worth closing. In practice this took one real build from 117 changes included to 128.
